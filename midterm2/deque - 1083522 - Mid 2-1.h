...
public:
		...
		: myData()
	{
		...
		{
			myData.mapSize = 8; //add
			myData.mySize = right.myData.mySize; //add
			size_type newFront = (right.myData.myOff - 1) % (4 * right.myData.mapSize);
			if (right.myData.mySize > 4 * (right.myData.mapSize - 1)) //modify
			{
				while (myData.mySize > 4 * (myData.mapSize - 1)) //add
					myData.mapSize *= 2; //add

				MapPtr tmp = new value_type * [myData.mapSize](); //modify
				size_type val = myData.mapSize; //modify
