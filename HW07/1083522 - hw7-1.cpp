	...
	vector& operator=(const vector& right)
	{
		if (this != &right) // avoid self-assignment
		{
			...
			if (rightSize > capacity())
			{
				...
				pointer tmp = new value_type[newCapacity]();
				...
				myData.myFirst = tmp;
				myData.myLast = tmp + rightSize;
				myData.myEnd = tmp + newCapacity;
			}
			else
			{
				for (size_type i = size(); i < rightSize; ++i)
...
bool HugeInteger< T >::operator<(HugeInteger& right)
{
	...
	typename T::iterator r_it = right.integer.end() - 1;

	for (; it != integer.begin()-1; --it, --r_it)
	{
		if (*it < *r_it)
			return true;
		else if(*it > *r_it)
...
HugeInteger< T > HugeInteger< T >::square(value_type powerTwo)
{
	...
	{
		for (t_it = integer.begin(),s_it = square.integer.begin()+j; t_it != integer.end(); ++t_it,++s_it)
		{
			size_t tmp = *it * *t_it;
			*s_it += tmp;
		}
	...
	for(s_it = square.integer.end()-1;s_it != square.integer.begin()-1;--s_it)
	{
		if (*s_it == 0)
...
HugeInteger< T > HugeInteger< T >::squareRoot(value_type powerTwo)
{
	...
	typename T::iterator s_it = sqrt.integer.end() - 1;
